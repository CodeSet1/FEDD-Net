...
try:
    ...
    import model.IDA.model.networks as networks
...
class DDPM(BaseModel):
    ...
    def get_current_visuals(self, sample=False):
        ...
        if sample:
            ...
            out_dict['DDPM'] = self.SR.detach().unsqueeze(0).float().cpu()
            out_dict['DDPM_single'] = self.SR.detach().float().cpu()
            ...
            out_dict['enlight'] = (self.data['R'].squeeze(0) * self.data['high'].squeeze(0)).detach().float().cpu()
            out_dict['R'] = self.data['R'].detach().float().cpu()
