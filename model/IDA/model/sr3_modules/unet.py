...
class UNet(nn.Module):
    def __init__(
        ...
        if with_noise_level_emb:        #创建一个噪声级别嵌入的MLP层，用于处理噪声级别信息。
...
#######下采样路径########
        num_mults = len(channel_mults)      #通道倍增因子的数量1，2，4，8，8
        pre_channel = inner_channel         #初始化为内部通道数
        feat_channels = [pre_channel]       #记录每个下采样阶段的特征通道数
        downs = [nn.Conv2d(in_channel, inner_channel,
                           kernel_size=3, padding=1)]       #列表存储下采样路径中的所有层
...
class UNet_banch(nn.Module):        #U-Net的变体
    def __init__(
    ...
    def forward(self, x, time):
        x_c = x[:, 1:2, :, :]       #从x中提取第二个通道的所有数据————>（B,1,H,W）
        x_n = x[:, 0:1, :, :]       #从x中提取第一个通道的所有数据————>（B,1,H,W）

        t = self.noise_level_mlp(time) if exists(
            self.noise_level_mlp) else None         #噪声级别计算

        x_n_enc_level1 = self.proj_x(x_n)
        x_c_enc_level1 = self.proj_c(x_c)       #特征编码
        ...
        out_x_n_enc_level1 = out_x_n_enc_level1 + out_x_c_enc_level1        #特征融合

        inp_x_n_enc_level2 = self.x_down1_2(out_x_n_enc_level1)     #下采样
        ...
        out_x_n_enc_level2 = out_x_n_enc_level2 + out_x_c_enc_level2        #特征融合

        inp_x_n_enc_level3 = self.x_down2_3(out_x_n_enc_level2)     #下采样
        ...
        inp_x_c_enc_level3 = self.c_down2_3(out_x_c_enc_level2)     #下采样
        for layer in self.c_encoder_level3:
                inp_x_c_enc_level3 = layer(inp_x_c_enc_level3)
        out_x_c_enc_level3 = inp_x_c_enc_level3
        out_x_n_enc_level3 = out_x_n_enc_level3 + out_x_c_enc_level3        #特征融合

        inp_x_n_enc_level4 = self.x_down3_4(out_x_n_enc_level3)     #下采样
        ...
        inp_x_c_enc_level4 = self.c_down3_4(out_x_c_enc_level3)     #下采样
        for layer in self.c_encoder_level4:
                inp_x_c_enc_level4 = layer(inp_x_c_enc_level4)
        out_x_c_enc_level4 = inp_x_c_enc_level4
        out_x_n_enc_level4 = out_x_n_enc_level4 + out_x_c_enc_level4        #特征融合
        ...
        inp_x_n_dec_level3 = self.up4_3(out_x_n_dec_level4)         #上采样
        inp_dec_level3 = torch.cat([inp_x_n_dec_level3, out_x_n_enc_level3], 1)     #跳跃连接
        ...
        inp_x_n_dec_level2 = self.up3_2(out_x_n_dec_level3)      #上采样
        inp_dec_level2 = torch.cat([inp_x_n_dec_level2, out_x_n_enc_level2], 1)     #跳跃连接
        ...
        inp_x_n_dec_level1 = self.up2_1(out_x_n_dec_level2)         #上采样
        inp_dec_level1 = torch.cat([inp_x_n_dec_level1, out_x_n_enc_level1], 1)     #跳跃连接
...
class LightBrightnessAdjustment(nn.Module):     #用于调整图像的亮度和亮度噪声水平
