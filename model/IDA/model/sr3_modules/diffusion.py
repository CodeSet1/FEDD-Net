...
#############################################
        lvlb_weights_eps = self.betas ** 2 / (2 * self.posterior_variance * to_torch(alphas) * (1 - self.alphas_cumprod))       #计算lvlb_weights_eps权重向量
        lvlb_weights_x0 = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod))     #计算lvlb_weights_x0权重向量
        lvlb_weights_eps[0] = lvlb_weights_eps[1]
        lvlb_weights_x0[0] = lvlb_weights_x0[1]     #处理边界情况，将第一个元素设置为第二个元素的值，确保权重在第一个时间步长上有一个合理的值
        self.register_buffer('lvlb_weights_eps', lvlb_weights_eps, persistent=False)
        self.register_buffer('lvlb_weights_x0', lvlb_weights_x0, persistent=False)      #注册模型缓冲区，缓冲区是模型的一部分，但不会被视为模型参数，也不会在训练过程中更新
        assert not torch.isnan(self.lvlb_weights_eps).all()
        assert not torch.isnan(self.lvlb_weights_x0).all()      #检查NaN值
#############################################
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None):
        ...
        x_recon = self.Unet_fn(torch.cat([condition_x, x_recon], dim=1), noise_level)
        if clip_denoised:
            x_recon.clamp_(-1., 1.)
    ...
    @torch.no_grad()
    def p_sample_loop_ddim(self, x_in, continuous=False):
        device = self.betas.device
        ...
        for i in tqdm(reversed(range(ddim_timesteps)), desc='Sampling loop Diff-IDA time step', total=ddim_timesteps):
        ...
        return (ret_img, x0_img, denoise_x0_img) if continuous else (ret_img[-1], x0_img, denoise_x0_img)
...
####################更好地处理正负误差的不平衡问题#########################
    def unbalanced_L1_loss(self, I_pred, I_real):
        alpha = 2
        diff = I_real - I_pred      #计算预测值和真实值之间的插值
        mask = diff < 0     #创建掩码，用于标记插值小于0的位置
        loss = alpha * torch.sum(torch.abs(diff) * mask) + torch.sum(torch.abs(diff) * ~mask)       #计算损失：torch.sum(torch.abs(diff) * mask)：计算差值绝对值在掩码为 True 的位置上的和；torch.sum(torch.abs(diff) * ~mask)：计算差值绝对值在掩码为 False 的位置上的和

        return loss
#正误差（预测值小于真实值）和负误差（预测值大于真实值）可能对最终结果的影响不同，因此需要根据实际情况调整对正负误差的重视程度。
#########################################################################
