...
if __name__ == "__main__":
    ...
    parser.add_argument('-c', '--config', type=str, default='config/Diff_HDR_val.json',
    ...
    Time_mat = np.zeros(len(val_loader))
    ...
    for _,  val_data_collect in enumerate(val_loader):
        ...
        end_time = time.time()
        Time_mat[idx-1] = (end_time - start_time)
        visuals = diffusion.get_current_visuals()

        hr_img = Metrics.tensor2img(visuals['high'])  # uint8
        x_0_img = Metrics.tensor2img(visuals['x_0'])  # uint8
        fake_img = Metrics.tensor2img(visuals['DDPM'])  # uint8
        fake_single = Metrics.tensor2img(visuals['DDPM'][-1])  # uint8

        img_mode = 'grid'
        if img_mode == 'single':
            # single img series
            single_img = visuals['DDPM']  # uint8
            ...
            Metrics.save_img(
                hr_img, '{}/{}_{}_high.png'.format(result_path, current_step, name))
            Metrics.save_img(
                x_0_img, '{}/{}_{}_x_0.png'.format(result_path, current_step, name))
            Metrics.save_img(
                fake_img, '{}/{}_{}_inf.png'.format(result_path, current_step, name))"""
            Metrics.save_img(
                fake_single, '{}/{}.png'.format(result_path, name))

    #sio.savemat('./' + 'Time' + '.mat', {'output': Time_mat})
