...
class DTCWTTransform(nn.Module):
    ...
    def forward(self, x):
        """
        正变换: 将输入张量进行 DTCWT 分解。
        :param x: 输入张量，形状为 (batch_size, channels, height, width)。
        :return: 低频部分和高频部分。
        """
        batch_size, channels, height, width = x.shape
        lowpass_list = []
        highpass_list = [[] for _ in range(self.levels)]
        ...
        for b in range(batch_size):
            ...
            for c in range(channels):
                ...
                # 保存高频部分
                for l, highpass in enumerate(coeffs.highpasses):
                    highpass_tensor = torch.tensor(highpass, device=x.device)
                    if len(highpass_list[l]) <= b:
                        highpass_list[l].append([])
                    highpass_list[l][b].append(highpass_tensor)
        ...
        highpasses = [
            torch.stack(
                [torch.stack(channels, dim=0) for channels in level],
                dim=0,
            )
            for level in highpass_list
        ]
    ...
    def inverse(self, lowpass, highpasses):
        """
        逆变换: 将 DTCWT 分解的低频和高频部分组合为原始图像。
        :param lowpass: 低频部分，形状为 (batch_size, channels, low_h, low_w)。
        :param highpasses: 高频部分的列表，每个元素对应一个尺度。
        :return: 重建后的张量。
        """
        ...
        for b in range(batch_size):
            ...
            for c in range(channels):
                # 准备单个通道的低频和高频分量
                lowpass_np = lowpass[b, c].cpu().numpy()
                highpasses_np = [highpass[b, c].cpu().numpy() for highpass in highpasses]
...
class TDN(nn.Module):
    ...
    def forward(self, inp_img):
        lowpass1, highpasses1 = self.dtcwt_transform(inp_img)  # DTCWT分解
        restored_img1 = self.dtcwt_transform.inverse(lowpass1, highpasses1)  # iDTCWT重建

        inp_enc_level1 = self.patch_embed(restored_img1)
...
########################解码阶段########################
#通过解码器层对编码后的特征图进行解码，并在每一步中融合编码器对应层的特征
...
########################################################
        out_dec_level1 = self.output(out_dec_level1)        #细化阶段：对解码器的输出进行细化处理
