...
def train_one_epoch(model, optimizer, lr_scheduler, data_loader, device, epoch, filefold_path):
    model.train()
    ...
    save_epoch = 10  # 每10个epoch保存一次训练集的结果

    if epoch % save_epoch == 0:
        trainfold_path = os.path.join(filefold_path, f"train_epoch_{epoch}")
        if os.path.exists(trainfold_path) is False:
            os.makedirs(trainfold_path)
    ...
    for step, data in enumerate(data_loader):
        ...
        if epoch % save_epoch == 0:
            ...
            save_pic(R_low_img, trainfold_path, f"{step}_R_low")
            save_pic(R_high_img, trainfold_path, f"{step}_R_high")
            save_pic(L_low_img, trainfold_path, f"{step}_L_low")
            save_pic(L_high_img, trainfold_path, f"{step}_L_high")
